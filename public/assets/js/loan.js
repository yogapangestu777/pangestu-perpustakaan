$(document).ready(function () {
                    ...
                    <div class="dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-end">
                            <button class="dropdown-item" data-bs-toggle="modal" data-bs-target="#loanModal"
                                ...
                                data-method="PUT"
                                data-member="${loan.id_anggota}"
                                data-book="${loan.id_buku}"
                                data-loan-date="${loan.tanggal_pinjam}"
                                data-return-date="${loan.tanggal_kembali}">
    ...
    $("#loanModal").on("show.bs.modal", function (event) {
        ...
        if (option.method === 'PUT') {
            $("#loanMember").val($button.data("member") || '').trigger('change');
            $("#loanBook").val($button.data("book") || '').trigger('change');
            $("#loanLoanDate").val($button.data("loan-date") || '');
            $("#loanReturnDate").val($button.data("return-date") || '');
        } else {
            resetModal();
        }
    });
    ...
    $("#saveLoan").on("click", function () {
        const postData = {
            member: $("#loanMember").val(),
            book: $("#loanBook").val(),
            loanDate: $("#loanLoanDate").val(),
            returnDate: $("#loanReturnDate").val()
        };
        $.ajax({
            url: option.action,
    ...
    $('#loanModal').on('hidden.bs.modal', resetModal);
